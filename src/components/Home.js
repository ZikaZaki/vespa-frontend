    ...
    <div 
      ...
      <div className="flex flex-row-reverse justify-between align-middle items-center">
        ...
        <div id="hamburger" className="flex flex-col cursor-pointer w-8 h-5 gap-2">
          <span  className="bg-white w-full h-4 rounded-full"></span>
          <span className="bg-white w-full h-4 rounded-full"></span>
        </div>
      </div>
      <div className="flex flex-col h-full w-full items-center justify-center">
        ...
        <div id="login_signup_buttons" className="flex flex-col w-full p-2 gap-8 md:flex-row md:w-[30rem] md:p-4">
          <button 
            type="button"
            className="w-full h-[3rem] bg-very_dark_limerick text-2xl text-white font-semibold rounded-full text-center opacity-90 hover:opacity-100 hover:shadow-lg focus:bg-very_dark_limerick focus:shadow-lg focus:outline-none focus:ring-0 active:bg-very_dark_limerick active:shadow-lg transition duration-150 ease-in-out md:min-w-[12rem]" onClick={() => showModal('LOGIN')}>
            Log In
          </button>
          <button 
            type="button" 
            className="w-full h-[3rem] bg-very_dark_limerick text-2xl text-white font-semibold rounded-full text-center opacity-90 hover:opacity-100 hover:shadow-lg focus:bg-very_dark_limerick focus:shadow-lg focus:outline-none focus:ring-0 active:bg-very_dark_limerick active:shadow-lg transition duration-150 ease-in-out md:min-w-[12rem]" onClick={() => showModal('REGISTER')}>Sign Up
          </button>
        </div>
      </div>
      ...
      {showLogin || showRegister ? (
        <>
          {/* Main modal */}
          <div
            aria-hidden="true" className="fixed top-0 left-0 right-0 z-50 flex flex-col items-center justify-center w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-full backdrop-blur-sm">
            <div className="relative w-full h-modal max-w-md md:h-auto">
              ...
              <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                {
                  showLogin ? (
                    <Login handleSuccessfulAuth={handleSuccessfulAuth} showModal={showModal} />
                  ) : null 
                }
                {
                  showRegister ? (
                    <Registration handleSuccessfulAuth={handleSuccessfulAuth} showModal={showModal} />
                  ) : null 
                }
              </div>
            </div>
